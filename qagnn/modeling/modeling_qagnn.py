...
class QAGNN_Message_Passing(nn.Module):
    ...
    def forward(self, H, A, node_type, node_score, cache_output=False):
        ...
        _X = X.view(
            -1, X.size(2)
        ).contiguous()  # [`total_n_nodes`, d_node] where `total_n_nodes` = b_size * n_node
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        adj = (
            edge_index.to(node_type_ids.device),
...
class LM_QAGNN_DataLoader(object):
    ...
    def __init__(
        ...
        # print(MODEL_NAME_TO_CLASS.items())
        # sys.stdout.flush()
        ...
        *self.dev_decoder_data, self.dev_adj_data = (
            load_sparse_adj_data_with_contextnode(
                dev_adj_path, max_node_num, num_choice, args
            )
        )
        assert all(
            len(self.train_qids) == len(self.train_adj_data[0]) == x.size(0)
            for x in [self.train_labels]
            + self.train_encoder_data
            + self.train_decoder_data
        )
        assert all(
            len(self.dev_qids) == len(self.dev_adj_data[0]) == x.size(0)
        ...
        if test_statement_path is not None:
            ...
            *self.test_decoder_data, self.test_adj_data = (
                load_sparse_adj_data_with_contextnode(
                    test_adj_path, max_node_num, num_choice, args
                )
            )
            assert all(
                len(self.test_qids) == len(self.test_adj_data[0]) == x.size(0)
...
class GATConvE(MessagePassing):
    ...
    def forward(
        ...
        aggr_out = self.propagate(
            edge_index, x=x, edge_attr=edge_embeddings
        )  # [N, emb_dim]
    ...
    def message(self, edge_index, x_i, x_j, edge_attr):  # i: tgt, j:src
        ...
        query = self.linear_query(x_j).view(
            -1, self.head_count, self.dim_per_head
        )  # [E, heads, _dim]
